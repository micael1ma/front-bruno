...
const chat = () => {
  ...
  useEffect(() => {
    ws = new WebSocket("ws://192.168.18.15:3000");
  ...
  const sendMessage = () => {
    const jsonMsg: any = { text: message, sentBy: userLogged };
    const jsonString: string = JSON.stringify(jsonMsg);
    chat.messages.push(jsonMsg);
    setChat({ messages: chat.messages });
    ws.send(jsonString);
    setMessage("");
  };
